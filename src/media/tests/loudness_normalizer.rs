...
use crate::media::{
    AudioFrame, Samples, loudness_normalizer::LoudnessNormalizer, processor::Processor,
};
...
fn test_basic_processing() {
    ...
    let mut normalizer = LoudnessNormalizer::new(sample_rate).unwrap();
    let mut out_file = File::create("fixtures/hello_book_course_zh_16k_normalized.pcm.decoded").unwrap();
    ...
    for chunk in all_samples.chunks(frame_size) {
        ...
        normalizer.process_frame(&mut frame).unwrap();
    ...
    println!(
        "ffplay -f s16le -ar {} fixtures/hello_book_course_zh_16k_normalized.pcm.decoded",
        sample_rate
    );
}
...
fn test_silence_does_not_pump_gain() {
    ...
    let gain_after_speech = normalizer.current_gain_for_test();
    ...
    let gain_after_silence = normalizer.current_gain_for_test();
    ...
    let gain_after_noise = normalizer.current_gain_for_test();
...
#[test]
fn test_loudness_normalizer_performance() {
    if cfg!(debug_assertions) {
        println!("Skipping loudness normalizer performance test in debug mode.");
        return;
    }
